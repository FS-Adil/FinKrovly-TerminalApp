...
// Страница перемещения рулона
function RollMovementPage() {
...
const [formData, setFormData] = React.useState({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSearchTerm('');
      // Обновляем список доступных рулонов
      fetchFormData();
    } catch (error) {
      console.error('Ошибка при оформлении перемещения:', error);
      alert('Не удалось оформить перемещение');
    }
  };
      ...
      <form onSubmit={handleSubmit} className="movement-form">
        ...
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <Select
              ...
              value={workshops.find(w => w.id === formData.toWorkshop) ? 
                { value: formData.toWorkshop, label: workshops.find(w => w.id === formData.toWorkshop).name } : 
                null}
              onChange={(selected) => handleSelectChange('toWorkshop', selected)}
        ...
        <div className="form-group">
          ...
          <Select
            ...
            value={employees.find(e => e.id === formData.responsible) ? 
              { value: formData.responsible, label: employees.find(e => e.id === formData.responsible).name } : 
              null}
        ...
        <div className="form-info">
          <h3>Информация о рулоне:</h3>
          <div className="info-grid">
            {formData.rollNumber && rolls.find(r => r.number === formData.rollNumber) && (
              <>
                <div className="info-item">
                  <span className="info-label">Материал:</span>
                  <span className="info-value">
                    {rolls.find(r => r.number === formData.rollNumber).material}
                  </span>
                </div>
                <div className="info-item">
                  <span className="info-label">Ширина:</span>
                  <span className="info-value">
                    {rolls.find(r => r.number === formData.rollNumber).width} мм
                  </span>
                </div>
                <div className="info-item">
                  <span className="info-label">Длина:</span>
                  <span className="info-value">
                    {rolls.find(r => r.number === formData.rollNumber).length} м
                  </span>
                </div>
                <div className="info-item">
                  <span className="info-label">Дата производства:</span>
                  <span className="info-value">
                    {new Date(rolls.find(r => r.number === formData.rollNumber).productionDate).toLocaleDateString()}
                  </span>
                </div>
              </>
            )}
        ...
        <div className="form-buttons">
          <button type="submit" className="submit-btn" disabled={!formData.rollNumber}>
